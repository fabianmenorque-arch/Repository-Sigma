...
const VERSION_SCRIPT = "2026-07-30-v5-migracion-codigos";
...
function actualizarOrden_(body) {
  ...
  // Respaldo del lado del servidor: si pasa a "En Proceso" y todavía no
  // tiene Fecha_Inicio (el cliente no la mandó, o se actualizó desde otro
  // lugar), se completa acá para no perder el dato.
  let fechaInicio = body.Fecha_Inicio || ordenActual.Fecha_Inicio;
  if (nuevoEstado === "En Proceso" && !fechaInicio) {
    fechaInicio = new Date().toISOString();
    const colInicio = headers.indexOf("Fecha_Inicio");
    if (colInicio !== -1) sheet.getRange(fila, colInicio + 1).setValue(fechaInicio);
  }
  ...
  // el inicio REAL del trabajo (no la fecha de creación de la orden, que
  // puede ser muy anterior si la orden esperó por prioridades de producción).
