...
/* ---------------------------------------------------------------
   4b. PARADAS DE PROCESO — agrupación por área y helpers de fecha
   Compartido entre Análisis (carga/edición) y el Dashboard (lectura).
   --------------------------------------------------------------- */

const SIGMA_GRUPOS_PARADA = [
  { label: 'Mecánica', miembros: ['mecanico','trapiche','canchon'] }
];

function sigmaNormalizarTexto(s){
  return String(s ?? '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]/g,'');
}

/** Devuelve el área agrupada de una causa de parada (ej: "Trapiche" → "Mecánica"). */
function sigmaAreaParada(causa){
  const norm = sigmaNormalizarTexto(causa);
  for(const g of SIGMA_GRUPOS_PARADA){ if(g.miembros.includes(norm)) return g.label; }
  return causa;
}

/** Rango [inicio, fin] (Date) de un mes dado en formato "YYYY-MM". */
function sigmaRangoMes(mesStr){
...
/** Barra de navegación compartida (sidebar). activePage: "dashboard"|"ordenes"|"equipos"|"qr"|"informes"|"alta"|"gestion"|"analisis"|"novedades"|"bombas" */
function sigmaRenderNav(activePage, basePath = ""){
  sigmaSetFavicon(basePath);

  const iconos = {
    dashboard: '<svg viewBox="0 0 24 24"><path d="M4 15a8 8 0 1 1 16 0"/><path d="M12 15l3.4-4.4"/><circle cx="12" cy="15" r="1.1" fill="currentColor" stroke="none"/></svg>',
    equipos: '<svg viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="6" rx="1.6"/><rect x="3" y="14" width="18" height="6" rx="1.6"/><circle cx="7.2" cy="7" r=".9" fill="currentColor" stroke="none"/><circle cx="7.2" cy="17" r=".9" fill="currentColor" stroke="none"/></svg>',
    qr: '<svg viewBox="0 0 24 24"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14.5" y="14.5" width="2.6" height="2.6" rx=".5"/><rect x="19" y="14.5" width="2" height="2" rx=".4"/><rect x="14.5" y="19" width="2" height="2" rx=".4"/><rect x="18" y="18" width="3" height="3" rx=".5"/></svg>',
    ordenes: '<svg viewBox="0 0 24 24"><rect x="5" y="3" width="14" height="18" rx="2"/><path d="M9 3v3h6V3"/><path d="M8.5 13.4l2 2 4-4.4"/></svg>',
    informes: '<svg viewBox="0 0 24 24"><path d="M6 2h9l5 5v13a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2z"/><path d="M14 2v6h6"/><path d="M8 17l2.3-2.8 2 1.8L16 12"/></svg>',
    analisis: '<svg viewBox="0 0 24 24"><rect x="4" y="12" width="3.2" height="8" rx="1"/><rect x="10.4" y="7" width="3.2" height="13" rx="1"/><rect x="16.8" y="3" width="3.2" height="17" rx="1"/></svg>',
  ...
  const links = [
    ...
    {id:"novedades", href:`${basePath}novedades.html`, label:"Novedades"},
...
/* ============================================================
   Helpers de estilo para gráficos (Chart.js) — misma paleta violeta/
   teal/rosa del tema oscuro, con buen contraste sobre fondo oscuro.
   Usados por dashboard.html, analisis.html y las fichas de equipo.
   ============================================================ */
const SIGMA_CHART_PALETTE = ["#8b6cf2","#22d3c9","#ff6fa8","#b39ffb","#5cf0bd","#ffc35e","#6f4fe0","#7fe0da"];

/** Ejes (ticks + grilla) con buen contraste sobre el fondo oscuro de SIGMA. */
function sigmaChartAxis(){
  return { ticks:{ color:"#a79fc9" }, grid:{ color:"rgba(255,255,255,.06)" } };
}

/** Leyenda con buen contraste sobre el fondo oscuro de SIGMA. */
function sigmaChartLegend(extra){
  return { labels: Object.assign({ color:"#f3f0ff", font:{family:"Inter"} }, extra || {}) };
}

/** Degradé vertical semi-transparente para el relleno de gráficos de área/línea
    (línea sólida arriba, se desvanece hacia abajo — como el estilo de referencia). */
function sigmaAreaGradient(ctx, chartArea, colorHex){
  if(!chartArea) return colorHex;
  const g = ctx.createLinearGradient(0, chartArea.top, 0, chartArea.bottom);
  g.addColorStop(0, colorHex + "66");
  g.addColorStop(1, colorHex + "00");
  return g;
}

/** Degradé vertical para barras (color sólido abajo → tono claro arriba),
    para usar junto con borderRadius y así lograr el look tipo ecualizador. */
function sigmaBarGradient(ctx, chartArea, colorFrom, colorTo){
  if(!chartArea) return colorFrom;
  const g = ctx.createLinearGradient(0, chartArea.bottom, 0, chartArea.top);
  g.addColorStop(0, colorFrom);
  g.addColorStop(1, colorTo || "#b39ffb");
  return g;
}
