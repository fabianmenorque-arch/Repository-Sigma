...
└── equipos/
    ├── index.html            ← Índice buscable de los 216 equipos
    ...
    └── BV-....html            ← Los 216 archivos generados (uno por equipo)
```
...
- **Tipo de máquina** (1-2 letras): `B`=Bomba, `R`=Reductor, `CF`=Centrífuga, `TU`=Turbina/Turbo, `CP`=Compresor, `CT`=Cinta transportadora, `AG`=Agitador, `EL`=Elevador, `FL`=Filtro, `GU`=Gusano, y ~20 códigos puntuales más (ver `equipos/generar_codigos.py` si necesitás agregar un tipo nuevo).
...
> vivo desde Sheets filtrando por su código. Por eso regenerar los 216
...
- [ ] **Migración de códigos de equipo** (si ya tenías datos cargados con el esquema viejo):
      1. Reemplazá la pestaña `Equipos` por `equipos_para_sheets.csv` (ya trae los códigos cortos nuevos).
      2. Creá una pestaña `Migracion_Codigos` y pegá ahí `migracion_codigos.csv`.
      3. Actualizá `Code.gs` (tiene la función nueva `migrarCodigosEquipo`) y volvé a implementar.
      4. En el editor de Apps Script, elegí `migrarCodigosEquipo` en el desplegable de funciones y ejecutala una vez.
      5. Revisá el log (`Ver → Registros`) para confirmar cuántas filas se actualizaron en cada pestaña.
      6. Borrá la pestaña `Migracion_Codigos`.
...
- [ ] Crear la pestaña `Gestion_Tareas` (columnas arriba) y completar su gid
      en `SIGMA_CONFIG.GIDS`.
- [ ] Correr `autorizarDrive` una vez manualmente desde el editor de Apps
      Script (▶ Ejecutar) para habilitar el permiso de Drive — necesario
      para que las fotos de los informes se guarden de verdad.
